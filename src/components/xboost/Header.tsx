...
function Header({ }: Props) {
	...
	useEffect(() => {
		...
		const onScroll = throttle((evt) => {	
			...
			if((st < 120) && (dataRef.current.headerBg == 1)) {
				sectionRef.current.classList.add('header-transparent');
				sectionRef.current.classList.remove('header-solid');
				dataRef.current.headerBg = 0;
			}
			else if((st >= 120) && dataRef.current.headerBg == 0){
				sectionRef.current.classList.remove('header-transparent');
				sectionRef.current.classList.add('header-solid');
				dataRef.current.headerBg = 1;
			}
	...
	return (
		<section ref={sectionRef} className="fixed w-full z-30">
			<header className="top-header">
				...
				<nav className="">
					<div className="hidden md:flex items-center space-x-4 actions">
